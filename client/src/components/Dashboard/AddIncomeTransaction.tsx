...
export default class AddIncomeTransaction extends React.Component<AddIncomeTransactionProps, AddIncomeTransactionState> {
    state: AddIncomeTransactionState = {
    ...
    getOptions = () => {
        // returns income names in the format needed for the Select component
        return this.props.incomeNames.map((name, i) => {
            return {
                key: i,
                value: name,
                text: name
    ...
    submit = async () => {
        try {
            ...
            await this.props.onAddIncomeTransaction(this.state.name, parseFloat(this.state.amount), this.state.note, date.valueOf());
            this.setState({
                name: this.props.incomeNames[0],
                amount: "0.00",
                note: "",
    ...
    render(): React.ReactNode {
        return (
            <Form onSubmit={this.submit} error={this.state.errors.length > 0} style={{paddingTop: "1rem"}}>
                ...
                <Form.Field>
                    <DatePicker selected={this.state.date} onChange={this.changeDate} dateFormat="MMMM DD, YYYY" maxDate={moment()} todayButton="Today" />
                </Form.Field>
