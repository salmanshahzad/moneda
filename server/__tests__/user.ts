...
describe("/api/delete_transaction", () => {
    let server: request.SuperTest<request.Test>;
    ...
    it("sends 401 when not signed in", done => {
        request(app).post("/api/delete_transaction").expect(401, done);
    });

    it("sends 400 with invalid inputs", done => {
        server.post("/api/delete_transaction").expect(400, [
            "Please enter a valid transaction id."
        ], done);
    });

    it("sends 200 with valid inputs", done => {
        server.post("/api/delete_transaction").send({
